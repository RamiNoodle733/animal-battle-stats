...
First release (2.11.0): preserve the source, inspect current architecture and production, harden confirmed public API input failures and malformed authentication inputs, establish a documented reproducible battle-model preview and crawlable methodology, connect it to existing navigation, validate, bump the site version, commit and push main. Deliver coherent tested release checkpoints as work proceeds. No database migration is needed for this first slice.

Intentionally deferred to subsequent releases: completed evidence backfill, editorial matchup pages, homepage replacement, complete scenario engine, human/team models, persistent community/retention features and P4 systems. These remain scheduled below. In particular, do not publish indexed pair pages or human probabilities before evidence/model review. Existing gameplay remains available during incremental migration.
...
- Priority: P0. Status: in-progress. Dependencies: M00 inspection. Scope: input/auth fixes now; full hardening next.
...
- Priority: P1. Status: not-started. Dependencies: M01, M02. Scope: later foundation release.
...
Next implementation order: finish M01 distributed abuse controls and battle/tournament replay/race protections; advance M04 source metadata/migrations; M03 homepage/selectors; then calibrated size/environment-aware M05 and editorial-quality M06 pages. These are still requirements, not silently omitted features.
