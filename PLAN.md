...
Current release (2.13.0): harden public mutations with MongoDB-backed distributed abuse budgets, bounded and canonical inputs, atomic community and daily animal votes, compare-event deduplication, honest persisted community counters, server-owned tournament quit telemetry and an exact-snapshot prestige write. The new rate-limit collection/index is created through the existing Mongoose deployment path; no destructive data migration is required.
...
- Priority: P0. Status: in-progress. Dependencies: M00 inspection. Scope: input/auth, tournament integrity and distributed mutation-abuse slices implemented; body-size/CSRF/privacy review and remaining mutation audit next.
...
Next implementation order: finish M01 body-size, CSRF, public-voter privacy and remaining mutation audits; advance M04 source metadata/migrations; build the remaining M03 homepage sections/shared shell; then calibrated size/environment-aware M05 and editorial-quality M06 pages. These are still requirements, not silently omitted features.
