...
Current release (2.15.0): replace process-local login/signup attempt tracking with atomic distributed network and normalized-account budgets, remove password-login account-provider disclosure, and suppress internal IDs from public profiles. This is an authentication/response hardening release; no destructive data migration is required.
...
- Priority: P0. Status: in-progress. Dependencies: M00 inspection. Scope: input/auth, tournament integrity, distributed abuse including login/signup, application body budgets, cookie-origin, public community/profile privacy and production-safe errors implemented; final historical-review evidence reconciliation remains.
...
Next implementation order: reconcile M01 against every historical review finding and close its evidence matrix; advance M04 source metadata/migrations; build the remaining M03 homepage sections/shared shell; then calibrated size/environment-aware M05 and editorial-quality M06 pages. These are still requirements, not silently omitted features.
