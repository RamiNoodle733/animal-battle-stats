 ...
 * posted to Discord and, when configured, Slack.
 */
...
async function notifyDiscord(eventType, data, req = null) {
    try {
        ...
        // Discord always gets a delivery record (it has been the channel of
        // record); Slack only once its webhook is configured.
        const discordEventId = crypto.randomUUID();
        const slack = Boolean(getSlackWebhookUrl());
