...
Current release (2.14.0): complete the next API boundary slice with per-route application body budgets, origin enforcement for ambient cookie mutations, same-origin and rate-limited browser telemetry, privacy-safe comment/chat/feed serialization, and complete nested-reply deletion. This is a response-contract migration only; no destructive data migration is required.
...
- Priority: P0. Status: in-progress. Dependencies: M00 inspection. Scope: input/auth, tournament integrity, distributed abuse, application body budgets, cookie-origin and public community privacy slices implemented; remaining mutation/error audit next.
...
Next implementation order: finish M01 remaining mutation/error audit; advance M04 source metadata/migrations; build the remaining M03 homepage sections/shared shell; then calibrated size/environment-aware M05 and editorial-quality M06 pages. These are still requirements, not silently omitted features.
