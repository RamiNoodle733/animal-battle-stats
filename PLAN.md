...
Current release (2.17.0): establish the animal evidence boundary without fabricating citations. A validated per-field contract, additive storage model, empty versioned manifest, read-only 225-animal legacy audit, migration/rollback rules and visible profile evidence disclosure ship now. No production or legacy catalogue data is rewritten.
...
- Priority: P1. Status: in-progress. Dependencies: M01; M02 for profile output. Scope: missing-value semantics, evidence schema/storage/audit and honest profile disclosure implemented; cited evidence backfill, correction workflow and remaining profile modules next.
